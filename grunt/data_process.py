...
def data_fit_straight(data, column):
    ...
    data    :   panda dataframe
                ...
                another one must be numerical values
    ...
    Return
    ------
    data_with_fit   :   pandas dataframe
                        with fit column data
    '''
    #We can't fit over dates, so we create
    ##another quantity, days since oldest date
    mindate = min(data['date'])

    ##get the difference
    deltas = (data['date'] - mindate).to_numpy()/(24*3600*1e9)

    ##make the fit
    fit = scipy.stats.linregress(deltas.astype('float'), data[column].to_numpy().astype('float'))

    ###fit line
    y = fit.slope*deltas.astype('float') + fit.intercept

    ##add it to the dataframe
    data.insert(2,'Fit',y)

    return data, round(fit.slope, 2)
